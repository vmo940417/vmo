...
put(12, "조달금리 — 바스켓 매수 시 지급", 2.60, "%", YLD, "in",
    "레포(RP) 조달금리.  매수차익거래(바스켓매수+선물매도)에서 적용")
put(13, "담보운용금리 — 바스켓 매도 시 수취", 2.90, "%", YLD, "in",
    "역RP 운용금리.  매도차익거래(바스켓매도+선물매수)에서 적용")
...
put(18, "포지션 방향", "매수차익 (바스켓매수+선물매도)", "", None, "in",
    "매수차익 = 바스켓매수+선물매도   /   매도차익 = 바스켓매도+선물매수")
put(19, "방향계수", '=IF(LEFT($B$18,4)="매수차익",1,-1)', "", INT_,
    "매수차익 = +1,  매도차익 = −1")
...
put(23, "대차수수료 (매도차익 시에만)", 0.0, "bp", BP, "in",
    "바스켓을 빌려서 파는 매도차익거래에만 적용. 매수차익거래에서는 무시된다 "
    "(산 채권을 대차로 빌려주는 것은 별도 영업이며, special 이점은 조달금리 B12 에 이미 반영됨)")

dv = DataValidation(
    type="list",
    formula1='"매수차익 (바스켓매수+선물매도),매도차익 (바스켓매도+선물매수)"',
    allow_blank=True)
...
put(31, "▶ 적용금리 (방향 반영)", "=IF($B$19=1,$B$12,$B$13-$B$23/100)", "%", YLD, "key",
    "매수차익 → 조달금리 B12 그대로.  매도차익 → 담보운용금리 B13 − 대차수수료 B23")
...
put(73, "권장 포지션",
    '=IF(B68>0,"매수차익 (바스켓매수+선물매도)",IF(B68<0,"매도차익 (바스켓매도+선물매수)","중립"))',
    "", None, "key")
put(74, "판정", '=IF(B71>0,"차익거래 가능","비용 미달 — 관망")', "", None, "key")

put(75, "▶ 적용액면 기준 필요 계약수", '=IFERROR($E$58/($C$55+$D$55),0)', "계약", "0.00",
    "key", "실제 보유액면(57행)을 직접 넣었다면 B17 을 이 값으로 맞춰야 헤지가 성립한다")
...
# ── [5] 만기 손익 ───────────────────────────────────────────
put(81, "▶ 순 BPV 노출 (현물 − 선물)",
    "=C52/100*C58+D52/100*D58-$B$60*$B$16*$B$17", "원/1bp", '#,##0', "key",
    "0 이어야 정상. ±100bp 시 이 값의 100배가 그대로 손익으로 남는다")
as_text(m["D81"]).value = None
m["D81"] = ('=IF(ABS($B$81)<=$B$60*$B$16*$B$17*0.005,"○ 헤지 정합",'
            '"✕ 계약수(B17) 또는 액면(57행) 조정 필요")')
m["D81"].font = F_KEY
