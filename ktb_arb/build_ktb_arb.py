...
readme = [
    ...
    ("", "· 시나리오 손익의 현물 평가는 선도 BPV + 컨벡시티 2차 테일러 근사를 사용 (±100bp 범위에서 오차 극소)."),
...
CFV = {"C": "B", "D": "V", "E": "AP"}   # 바스켓 열 → 현금흐름 스칼라 값 열
...
for i, col in enumerate("CDE"):
    put(pl, 27 + i, f"종목{i+1} 만기 단가(더티)",
        f"=바스켓!{col}36-바스켓!{col}39*$B$24+0.5*바스켓!{col}41*($B$24/10000)^2", "", "0.0000")
...
pl["A52"].font = F_NOTE
pl["A53"] = "※ 27~29행 만기단가는 선도 BPV + 컨벡시티 2차 테일러 근사입니다. |shift| > 200bp 에서는 오차가 커집니다."
pl["A53"].font = F_NOTE
...
for i in range(21):
    ...
    sc.cell(row=R, column=5,
            value=f"=설정!$B$39*("
                  f"(-바스켓!C39*$A{R}+0.5*바스켓!C41*($A{R}/10000)^2)/100*바스켓!C49"
                  f"+(-바스켓!D39*$A{R}+0.5*바스켓!D41*($A{R}/10000)^2)/100*바스켓!D49"
                  f"+(-바스켓!E39*$A{R}+0.5*바스켓!E41*($A{R}/10000)^2)/100*바스켓!E49)").number_format = N_WON
...
put(sc, 31, "shift 0 기준 손익", "=G14", "원", N_WON, "key",
    lab_col=1, val_col=2, unit_col=3, note_col=4)
sc["A33"] = ("※ BPV 헤지가 정확하면 위 표의 '순 만기손익'은 shift와 무관하게 거의 평평해야 합니다. "
             "잔여 변동은 컨벡시티 불일치(2차 효과)이며, 이는 헤지로 제거되지 않는 구조적 잔여리스크입니다.")
sc["A33"].font = F_NOTE
