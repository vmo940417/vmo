...
> 참고: 이 저장소에는 쇼츠 파이프라인과 별개로, 브라우저에서 바로 돌아가는
> 농장 경영 게임 **[🌱 두더지 농장](game/README.md)** (`game/` 폴더)이 함께 들어 있습니다.
> 데스크톱/모바일 모두 지원하며, `game/mole-farm.html` **파일 하나만 내려받아 더블클릭**하면 바로 실행됩니다.
...
**항상 "다음" 03:00에만 공개됩니다.** `upload_youtube.py`의 `_compute_publish_at()`은
오늘 03:00이 이미 지났으면 즉시 공개하는 대신 **내일 03:00으로 예약을 넘깁니다.**
그래서 이 워크플로우가 실제로 몇 시에 실행되든 — 정시든, 몇 시간 지연이든 — 절대
아무 때나 즉시 공개되는 일이 없고 항상 정확히 03:00 정각에만 영상이 나갑니다.

**왜 이렇게 만들었나요?**
GitHub Actions의 `schedule`(cron) 트리거는 정시 실행을 보장하지 않습니다. 실제
운영에서 2시간, 심하면 **9시간 가까이** 지연되는 걸 확인했습니다 (GitHub 공식
문서에도 트래픽이 몰리는 시간대, 특히 매 시 정각(`:00`)에는 지연되거나 아예
건너뛸 수 있다고 명시돼 있습니다). 처음에는 "목표 시각보다 몇 시간 일찍 실행되도록
버퍼를 둔다"는 방식으로 대응했는데, 지연이 버퍼보다 커지면 여전히 즉시 공개로
새는 문제가 있었습니다. 그래서 버퍼를 늘리는 대신, **"이미 지났으면 즉시 공개"가
아니라 "이미 지났으면 다음 기회로 예약"** 하도록 로직 자체를 바꿔서, 워크플로우
실행 시각과 무관하게 03:00 정시 공개를 보장하도록 했습니다.

cron(`UTC 15:07`)은 그래도 목표 시각과 최대한 가깝게 맞추기 위한 것일 뿐이고,
정시 공개를 지키는 핵심 안전장치는 아닙니다. 필요하면 `cron` 값이나 `config.py`의
`PUBLISH_TIME_LOCAL`을 수정하세요.

**수동 테스트 시 결과를 바로 보고 싶으면** GitHub Actions **Run workflow**에서
`publish_now: true`를 켜세요. 예약 없이 즉시 공개로 올라가서 바로 확인할 수
있습니다 (`publish_now: false`가 기본값이며, 이 경우 항상 다음 03:00으로 예약됩니다).
